...
def predict(question, paragraphs, model, tokenizer, device, args):
    eval_examples = read_squad_examples(input_data={'question': question, 'paragraphs': paragraphs}, 
                                        is_training=False, 
    ...
    predicts = get_predictions(eval_examples, eval_features, all_results,
...
def read_squad_examples(input_data, is_training, version_2_with_negative):
    ...
    if type(input_data) == str:
        ...
        examples = []
        for p in paragraphs:    
            examples.append({'context': p, 'qas': [{u'answers': [],u'id': uuid.uuid4().hex,u'question':question, 
